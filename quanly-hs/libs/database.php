...
function db_connect(){
    global $conn;
    if (!$conn){
        $conn = mysqli_connect('localhost', 'root', '', 'quanly_hocsinh') 
...
// Hàm lấy danh sách, kết quả trả về danh sách các record trong một mảng - trả về 1 mảng
...
// Hàm lấy chi tiết, dùng select theo ID vì nó trả về 1 record - trả về 1 đối tượng là 1 dòng
...
// Hàm tạo chuỗi WHERE
function db_create_where($filter = array()) {
    // Chuỗi where
    $where = '';
...
// Hàm UPDATE table
function db_update($table, $data = array(), $wheredata = array() ) {
    // Chuỗi data
    $dataset = '';
     
    // Lặp qua biến $filter và bổ sung vào $data
    foreach ($data as $field => $value){
        if ($value != ''){
            $value = addslashes($value);
            $dataset .= " `$field` = '$value' ,";
    ...
    $datawhere = db_create_where($wheredata);

    // Tạo câu lệnh SQL
    $sql = "UPDATE {$table} SET {$dataset} {$datawhere}";
