...
<?php 
    ...
    // Tìm tổng số records
    // $sql = db_create_sql('SELECT count(id) as counter from hs_thongtin WHERE hotenhs like "'.$hotenhs.'"');
    $sql = db_create_sql("SELECT count(id) as counter from hs_thongtin WHERE hoten_hs like '%".$hotenhs."%'");
    ...
    FROM hs_thongtin WHERE hoten_hs like '%".addslashes($hotenhs)."%' ORDER BY malop ASC, mahs ASC, namhoc ASC  LIMIT {$paging['start']}, {$paging['limit']}");
    $hss = db_get_list($sql);
