...
<?php 
    ...
    // Tìm tổng số records
    $sql = db_create_sql("SELECT count(id) as counter from hs_thongtin WHERE hoten_bo like '%".$hotenph."%' or hoten_me like '%".$hotenph."%'");
    ...
    FROM hs_thongtin WHERE hoten_bo like '%".addslashes($hotenph)."%' or hoten_me like '%".$hotenph."%' ORDER BY malop ASC, mahs ASC, namhoc ASC  LIMIT {$paging['start']}, {$paging['limit']}");
    $hss = db_get_list($sql);
