...
<?php 
    ...
    // Tìm tổng số records
    $sql = db_create_sql('SELECT count(id) as counter from hs_thongtin {where}', array('malop' => $malop));
    ...
    <div class="filter-part filter__01">
        <form class="form" method="post" action="<?php echo create_link(base_url('admin/index.php'), array('m' => 'hs', 'a' => 'filter-malop')); ?>">
            <select name="filter-malop">
                ...
                <option value="<?php echo $lop['malop'];?>" <?php echo ($lop['malop'] == $malop) ? 'selected' : ''; ?> ><?php echo $lop['malop'];?></option>
                <?php } ?>
            </select>
