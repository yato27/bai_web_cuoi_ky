...
// Tìm tổng số records
    $sql = db_create_sql('SELECT count(hs_diem.mahs) as counter from hs_diem INNER JOIN hs_thongtin on hs_thongtin.mahs = hs_diem.mahs
                          WHERE hoten_hs like "%'.$hotenhs.'%"');
    ...
    FROM  hs_diem INNER JOIN hs_thongtin on hs_thongtin.mahs = hs_diem.mahs INNER JOIN monhoc on monhoc.mamh = hs_diem.mamh  WHERE hoten_hs like '%".addslashes($hotenhs)."%' ORDER BY malop ASC, monhoc.mamh ASC, mahs ASC  LIMIT {$paging['start']}, {$paging['limit']}");
