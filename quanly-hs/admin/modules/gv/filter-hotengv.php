...
// Tìm tổng số records
    $sql = db_create_sql("SELECT count(id) as counter from gv_thongtin WHERE hoten_gv like '%".$hotengv."%'");
    ...
    // Lấy danh sách gv
    $sql = db_create_sql("SELECT magv, hoten_gv, ngaysinh, gioitinh, chucvu, malop, namhoc, chuyennganh, sdt, diachi FROM gv_thongtin WHERE hoten_gv like '%".addslashes($hotengv)."%' LIMIT {$paging['start']}, {$paging['limit']}");
    $gvs = db_get_list($sql);
?>
