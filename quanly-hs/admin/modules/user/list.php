...
// Tìm tổng số records
    ...
    // Lấy danh sách User
    $sql = db_create_sql("SELECT * FROM tb_user {where} ORDER BY username asc LIMIT {$paging['start']}, {$paging['limit']}");
...
<table cellspacing="0" cellpadding="0" class="form tb_user">
    <thead>
        <tr>
            <td>Username</td>
            <td>Email</td>
    ...
    <tbody>
        ...
        <?php foreach ($users as $item){ ?>
            <tr>
                ...
                <td><?php if($item['level'] == 1) {echo 'Admin';} else {echo 'Member';} ?></td>
                ...
                <td>
                    <form method="POST" class="form-delete" action="<?php echo create_link(base_url('admin/index.php'), array('m' => 'user', 'a' => 'delete')); ?>">
                        ...
                        <a href="<?php echo create_link(base_url('admin'), array('m' => 'user', 'a' => 'edit', 'username' => $item['username'])); ?>">Edit</a>
                        
                        <input type="hidden" name="user_username_edit" value="<?php echo $item['username']; ?>"/> 
